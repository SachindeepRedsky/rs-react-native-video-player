...
const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  const pan = useRef(
    new Animated.ValueXY({ x: SCREEN_WIDTH - MINI_WIDTH - 10, y: SCREEN_HEIGHT - MINI_HEIGHT - 10 })
  ).current;
  ...
  const renderControls = () => (
    <View style={[styles.controlsWrapper, isFullscreen && styles.fullscreenOverlay]}>
      <Slider
        style={[isFullscreen ? styles.sliderFullscreen : styles.slider, customStyles.slider]}
        minimumValue={0}
        maximumValue={duration}
        value={currentTime}
        minimumTrackTintColor="#00aced"
        maximumTrackTintColor="#ccc"
        thumbTintColor="#00aced"
      ...
      <View style={[styles.controls, customStyles.controls]}>
        <TouchableOpacity onPress={toggleMute} style={[styles.button, customStyles.button]}>
          <Image
            ...
            source={muted ? customIcons.unmute || require('./assets/unmute.png') : customIcons.mute || require('./assets/mute.png')}
          />
        </TouchableOpacity>
        ...
        <TouchableOpacity onPress={togglePlayPause} style={[styles.button, customStyles.button]}>
          <Image
            style={styles.tinyLogo}
            source={paused ? customIcons.play || require('./assets/play1.png') : customIcons.pause || require('./assets/pause.png')}
          />
        </TouchableOpacity>
        ...
        <TouchableOpacity onPress={toggleLoop} style={[styles.button, customStyles.button]}>
          <Image
            style={styles.tinyLogo}
            source={loop ? customIcons.loop || require('./assets/loop.png') : customIcons.unloop || require('./assets/unloop.png')}
          />
        </TouchableOpacity>
  ...
  const videoPlayer = isMinimized ? (
    <Animated.View
      style={[styles.miniContainer, { transform: [{ translateX: pan.x }, { translateY: pan.y }] }]}
    ...
    <View style={isFullscreen ? styles.fullscreenContainer : [styles.container, customStyles.container]}>
      <View style={{ flex: 1 }}>
        {isFullscreen ? (
            ...
            <View style={styles.infoContainer}>
              ...
              {!!tags.length && (
                <View style={styles.tagContainer}>
                  {tags.map((tag, i) => (
                    <Text key={i} style={[styles.tag, customStyles.tag]}>#{tag}</Text>
                  ))}
                </View>
              )}
            </View>
